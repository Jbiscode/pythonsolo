...
class Attack_Unit(Unit):   #  자녀클래스(부모클래스)         attack_unit 은 unit에서 상속받은 내용으로 만들어짐     
    def __init__(self,name,hp,speed,damage):        # 공격유닛 생성
        # self.name = name
        # self.hp = hp
        Unit.__init__(self,name,hp,speed)         ### 상속한 내용을 이렇게 써줘야함
        self.damage = damage
        print("{0} 유닛이 생성되었습니다.".format(self.name))
        print("체력 : {0}, 공격력 : {1}".format(self.hp,self.damage))
...
class Flyable:   
    ...
    def fly(self,name,location):             #메소드 오버라이딩을 통해서 공중유닛 fly도 move로 실행하게끔
        print("{0} : {1} 방향으로 날아갑니다. [속도 : {2}]".format(name,location,self.flying_speed))

# 공중 공격 유닛클래스        
class Flyable_Attack_Unit(Attack_Unit,Flyable):       #다중 상속
    def __init__(self,name,hp,damage,flying_speed):
        Attack_Unit.__init__(self,name,hp,0,damage)  # 공중유닛 지상스피드는 0으로 처리
